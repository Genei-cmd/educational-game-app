...
function startGame(subject) {
    ...
    // Start timer with 25 seconds
    startTimer(25);  // Changed from 60 to 25
...
function timeUp() {
    ...
    showNotification('Time\'s up! 25 seconds have passed.', 'error');
...
function showScreen(screenId) {
    ...
    if (targetScreen) {
        ...
        if (screenId !== 'gameScreen' && window.gameTimer) {
            clearInterval(window.gameTimer);
        }
...
// Add the displayCurrentQuestion function
...
function displayMatching(question) {
    ...
    pairsContainer.className = 'matching-pairs';
    
    // Create and shuffle arrays for items and matches
    const items = question.pairs.map((pair, index) => ({
        text: pair.item,
        index: index
    }));
    
    const matches = question.pairs.map((pair, index) => ({
        text: pair.match,
        index: index
    }));
    
    // Shuffle matches array
    matches.sort(() => Math.random() - 0.5);
    
    // Create columns for items and matches
    ...
    matches.forEach(match => {
        ...
        matchDiv.textContent = match.text;
        matchDiv.dataset.index = match.index;
        matchesColumn.appendChild(matchDiv);
    });
    
    pairsContainer.appendChild(itemsColumn);
    pairsContainer.appendChild(matchesColumn);
    container.appendChild(pairsContainer);
    gameArea.appendChild(container);
    
    // Initialize matching game interaction
